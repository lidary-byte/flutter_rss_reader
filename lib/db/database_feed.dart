...
extension DatabaseFeed on FeedBean {
  ...
  Future<int> insert() async {
    ...
    return isar.writeTxn(() async {
      var insertId = await isar.feedBeans.put(this);
      item?.forEach((element) async {
        element.feedId = insertId;
      });
      isar.rssItemBeans.putAll(item ?? []);
      return insertId;
    });
  }
  ...
  static Future<List<FeedCategoryBean>> groupByCategoryFeedList() async {
    ...
    return groupBy(feeds, (feed) => feed.category).entries
        .map(
          (entry) => FeedCategoryBean(category: entry.key, feeds: entry.value),
        )
        .toList();
  }
  ...
  Future<void> updateAndChildToDb() async {
    final isar = await isarInstance;
    final List<RssItemBean> items = await isar.rssItemBeans
        .filter()
        .feedIdEqualTo(id)
        .findAll();
